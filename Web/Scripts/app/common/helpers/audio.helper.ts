namespace LiveMonitor {
	export abstract class AudioHelper {
		 ...
		 */
		public static prependWavHeader(data: ArrayBuffer): ArrayBuffer {
		 ...
		 */
		public static concatBytes(arr1: ArrayBuffer, arr2: ArrayBuffer): ArrayBuffer {
			const result: Uint8Array = new Uint8Array(arr1.byteLength + arr2.byteLength);
		 ...
		 */
		public static padBuffer(buffer: AudioBuffer): AudioBuffer {
			const curSample: Float32Array = new Float32Array(1);

			buffer.copyFromChannel(curSample, 0, 0);

			let wasPositive: boolean = curSample[0] > 0;
